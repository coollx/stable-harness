...
The reveal gate is `engine/observer.py:43 _label_revealed`: a task with no resolution date (a capture-the-flag challenge, for example) is revealed immediately, since there is no temporal ordering to protect (`observer.py:44-48`); a task with one is revealed exactly when its resolution timestamp is at or before the batch watermark. Two channels are reported each cycle — in-batch reveals, and tasks from *earlier* batches whose labels have only now resolved, appended to `revealed_supplement.jsonl`. This implements our grading rule (each task graded with the harness that existed when it arrived) with the delay the real world imposes rather than a simulated one.
...
- **The reveal gate** — `engine/observer.py:43` plus the `revealed_supplement.jsonl` channel. A working implementation of our grading rule that also handles labels arriving for *earlier* batches, which our setting explicitly allows ("labels may arrive late").
