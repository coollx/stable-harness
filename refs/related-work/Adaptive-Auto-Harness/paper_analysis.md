...
Short name used below: Adaptive Auto-Harness, the paper's own name for its system. Where this document says "harness", "layer", or "task stream" it uses those terms as `docs/framing.md` defines them; the paper's own vocabulary (evolver, solver, evolution loss, adaptation loss, harness tree, agentic routing) is kept in italics on first use and attributed to the paper.
...
**Step 2 — Solve.** The chosen branch is checked out into the solver workspace, giving the solver that branch's system prompt, skills, tool registry, and memory. The solver (Claude Sonnet 4.6, temperature 0, 80-turn cap) produces the answer. Each task is graded with the harness that existed when the task arrived; the harness is edited only afterward. This is implemented as a per-task release-time filter on what the solver may see (Appendix A).

**Step 3 — Reveal.** The *temporal-reveal gate* hands each task's evaluation signal to the *evolver* only after that task's real-world resolution date. A prediction market that resolves days after the trade is an unlabeled trajectory until then; CTF-Dojo tasks have no resolution date, and the released code reveals their labels to the *evolver* immediately, since there is no temporal ordering to protect (`agent_evolve/engine/observer.py:44-48`; config `temporal_reveal`, Appendix B).
...
| Temporal-reveal gate | Labels reach the *evolver* only after the task's real-world resolution date | Notable — a concrete, reusable implementation of grading each task with the harness that existed when it arrived, on a stream whose labels genuinely arrive late |
...
|---|---|---|---|
| Adaptive Auto-Harness (this paper) | Leads all three streams; branch isolation prevents cross-regime contamination; delayed labels handled honestly | Still scores every edit on the batch that produced it; tree only grows (no code merges or retires a branch; the analyst prompt tells the model to mark a dragging branch for retirement, and nothing acts on the mark); about 6× the solver input tokens (233.2 M vs 35.3 M) and about 2.3× the wall-clock (59.5 vs 25.6 hours) of the no-evolution baseline on PolyBench (Table 5) | A term for future-stream performance; branch merge and retirement; an estimator for the losses it defines analytically |
...
## Discussion notes (2026-09-22)

Answers from a reading session with the researcher, each verified against the paper text or the released code. They extend, and in two places correct, the sections above.

**The analytical framework is a taxonomy in equation form, not a derivation and not a measurement.** Section 3.2 defines per-task regret against the best harness any builder could produce, inserts a middle reference (the best harness this builder class could produce if it saw the task first), and splits the regret into the two pieces around that reference. Proposition 1 is that telescoping identity; it holds for any middle point. Its only use in the paper is to label the two components: Section 3.3 opens "the decomposition identifies evolution loss" and presents the four-role builder, Section 3.4 opens "the decomposition identifies adaptation loss" and presents the branch tree and router. No code references either quantity; there is no oracle, estimator, objective term, or stopping rule derived from them. Neither loss is measurable as defined, and the limitations section says so ("analytical quantities, not directly estimated oracle losses"). The one number the framework produces is Table 11's replay gap between the best existing branch per task and the fixed main branch (37.5 points on CTF-Dojo, 8.8 on PolyBench), which is a lower bound on adaptation loss within the branches that happened to exist, on 40 to 80 tasks. Nothing in the paper estimates evolution loss; Figure 4 is ordinary sensitivity analysis. The formative evidence that actually motivates the design is Figure 1 and the `news_from_future.md` failure in the introduction. For our related-work text: "formalizes the gap to an oracle harness as a per-task regret, splits it into an evolver-capability term and a commit-before-seeing-the-task term, and uses the split to organize its two components; neither term is estimated, and neither represents the effect of an edit on later tasks."

**Heterogeneity is topic variation inside one task format; the three streams are never mixed.** Within PolyBench every task has the same shape (read a market question, decide, trade) and only the topic mix varies over time (politics, sports, finance, crypto, entertainment); CTF-Dojo varies by challenge category, FutureX by domain and language. Each benchmark is a separate experiment with its own seed and its own git repository, so a prediction market never follows a security challenge in one stream. Figure 11 is the only place different task kinds meet, and there the harnesses are mixed, not the tasks (see below). Across our reference set, only AgentStream (2608.00155) interleaves task kinds in one run, and it proposes no method; Harness Continual Learning (2608.19013) runs different kinds in ordered blocks.

**Solve-time adaptation is one model call.** The router (Claude Sonnet 4.6, temperature 0, 256 output tokens) receives the task text and, for every viable branch, its name, a one-line description, and the first 1,000 characters of its `README.md`, and returns a JSON object with a branch name, a confidence, and a reason (`algorithms/navigation/prompts.py:21`, `engine.py:138`). The system prompt is five rules: read each README's "When to route here" section, match positive and negative signals, route with confidence of at least 0.7 if exactly one branch matches strongly, otherwise main. Code overrides any non-main choice below 0.7 to main. The docstring says the router reads prompts, skills, and tools; the code reads only the README, which the builder itself wrote at build time following a template that demands a "When to route here" section. So the routing rule is authored in prose during evolution and applied at solve time. On the Table 10 subsets the fallback never fired. Routing is within one benchmark, among branches the builder created for that benchmark's sub-types (CTF-Dojo: crypto, rev, pwn, misc, web, forensics; PolyBench: sports, finance, culture, politics-world; FutureX: lvl1, lvl2, lvl3, a difficulty split the builder chose on its own).

**The harness tree has depth one, and edits are incremental on every branch.** Every branch is created by `git checkout -b <name> main` (`structured_navigation.py:580`, `versioning.py:277`); the branch record has no parent field and no code creates a branch from a branch. One evolution cycle (`structured_navigation.py:560-620`): check out main and run build-and-verify on its current content; then for each branch the analyst assigned work to, create it from main if new, otherwise rebase it onto main if main changed (on conflict, abort and copy only `tools/` and `infra/` from main, leaving the branch's prompts untouched); check out the branch and run build-and-verify on its current content using only the research for its regimes; write the README the router reads. Nothing flows back: `merge_branch` has no caller. The branch trigger handed to the analyst is capacity (main's prompt at its 10,000-character cap), secondarily regime-conditional rules already in main's prompt to be extracted into a branch. So their only content move is main to new branch at branch birth, made to relieve prompt size; there is no promotion in our sense.

**What is optimized: four text-file layers. What is fixed: the solver loop, its tools, the model.** Each benchmark's `manifest.yaml` declares the same evolvable layers: `prompts/system.md` (capped at 10,000 characters), `skills/<name>/SKILL.md` files, `tools/*.py` scripts listed in `registry.yaml`, and `memory/*.jsonl` entries; an `infra/` directory for network pipelines exists with its own flag but is not listed as a layer. The solver assembles them into one system prompt on every task: the prompt text, then every skill's full body, then each tool's name and description, then the last 10 memory entries (`agents/polybench/agent.py:62-93`). Skills are therefore neither executable nor retrieved; they are the system prompt under other file names, and only the `system.md` file has a size cap. The solver is fixed Python: a tool-calling loop with bash and submit (plus a step-by-step reasoning tool on PolyBench), an 80-turn limit, a sandbox, and Claude Sonnet 4.6. In our layer vocabulary they edit the system prompt, skill, tool-implementation, and long-term-memory layers; tool descriptions, middleware, sub-agent configuration, and weights are out of reach. The seed is one system prompt (27, 24, and 114 lines), an empty tool registry, and an empty memory file; the PolyBench seed prompt is eleven trading rules and a six-step reasoning recipe. Table 4 says the FutureX seed includes an infrastructure directory; the shipped seed has none, though its prompt already tells the solver to run `/infra/search_pipeline.py`. PolyBench's memory layer was switched off for the paper runs (`evolve_memory: false`, config comment: memory accreted about 30 KB of single-market notes never loaded usefully).

**Figure 11, explained.** A workspace is one finished harness from one full run. The paper hands each of the three finished workspaces to the solver on PolyBench tasks, then hands it a fourth harness made by combining all three. CWR is profit divided by confidence-weighted investment over traded markets, in percent. Results: PolyBench-built +60, FutureX-built +32, CTF-Dojo-built +15, combined +3; the untouched seed earns about +1.7 return in Table 2. So the combined harness is worse than any of its parts, including the security-challenge harness alone. Because every skill body is pasted into the prompt on every task, combining means the solver reads all three harnesses at once and the PolyBench-specific instructions are buried. The paper does not say how the combination was assembled, how many tasks were used, or whether the gap is stable across seeds; no confidence interval is given and the repository has no script for it. For us it is the cautionary case for promotion implemented as concatenation, and the paper's plainest measurement of harness content that helps alone and hurts in combination.

**The four-role evolver is prompting.** Each phase is a fresh Claude Opus 4.6 conversation with its own system prompt (45, 39, 34, and 28 lines in `templates/prompts/*_system.md`) and one tool, a bash command in a Docker sandbox with the harness mounted writable and the solver transcripts read-only (`aevolve/tools.py:15`); a second tool asks a human and is off in every main run. Analyze reads transcripts and revealed outcomes and writes `task_board.md`; Research runs up to three agents in parallel, one per regime, and writes `research_log.jsonl` with a works-or-not flag per approach; Build edits the harness files and updates `architecture.md`; Verify writes and runs test scripts and replies with a verdict that the code parses by looking for the string "VERDICT: PASS" (`structured_evolution.py:883`). Failure rolls the harness back to a pre-build git tag and retries, at most three times. Three mechanical steps follow: truncate the prompt to 10,000 characters, execute each registered tool once and delete it if it errors, strip search-count limits. The verifier never runs the solver on a task; it checks that the built artifacts execute on a sample query. Whether the edited harness solves more tasks is first learned on the next batch, after the edit is live. The cross-cycle state the ablation credits with the largest effect is the three scratch files. For us: there is no step where a score enters, so a commit-time check against previously passed tasks is a fifth step inserted between Build and commit, not a modification of an existing one.
