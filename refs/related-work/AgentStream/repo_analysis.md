...
**No score, no reward, and no gate is passed to `run_evolver`** (signature at `evolver.py:339-347`). Whatever the evolver writes is applied; the only rollback path is an exception. Scoring happens outside the agent entirely, in the benchmark's evaluator, and is never routed back. Each task is graded with the harness that existed when it arrived, and the edit that follows is chosen without any score; this is confirmed at the level of a function signature rather than inferred from prose.
...
- The per-task metrics record and `get_memory_tokens` in `scripts/*/run_experiment.py` — gives us per-task scores and `cumulative_avg_score`, a decline readout once compared with a reference run on the same tasks, and a harness-size series in the same file, which is precisely what the equal-harness-size control in our falsifier needs.
