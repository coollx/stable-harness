...
- Regime: a heterogeneous, non-stationary task stream. Each task is graded with the harness that existed when the task arrived; the harness is edited only afterward. Labels may arrive late: on a prediction market the true answer is known only when the event happens, days after the task was answered.
- Objective, per edit: $\max_{a_t}\ r_t(h_t, a_t)\ \ \text{s.t.}\ \ \Phi(h_{t+1}) \ge \Phi_{\min}$, where $r_t$ is task performance on the current batch and $h_{t+1}$ is the harness after the edit. Its relaxation $r_t + \gamma\,\Phi(h_{t+1})$ is the per-step reward of a Markov decision process over harness states, whose stream objective is $\mathbb{E}\big[\sum_t \gamma^t r_t\big]$. The horizon is set by $\gamma$; nothing slides. (open: $\gamma$, including whether and how the user sets it as a horizon preference, $\gamma = 0$ for short-term use and a high $\gamma$ for a long-lived deployment; the retention constraint at commit, hard versus soft.)
...
- Measurement (open): no harness paper in refs measures intransigence, the loss of the ability to keep patching; designing that measurement is a candidate main contribution.
...
| intransigence | How much the edits accumulated so far hurt a harness's ability to learn a new task, compared with a harness that starts from scratch; the loss of the ability to keep patching. Term from Riemannian Walk (1801.10112). |
